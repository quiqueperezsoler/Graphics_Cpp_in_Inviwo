...
void Scene::init(const ivec2& imageSize) { imageSize_ = imageSize; }
...
void Scene::drawCameraGeometry(std::shared_ptr<BasicMesh> mesh,
    ...
    Util::drawLineSegment(camPos_, bottomLeft_, vec4(1, 0.5, 0, 1), indexBuffer.get(), vertices);
    Util::drawLineSegment(camPos_, topLeft_, vec4(1, 0.5, 0, 1), indexBuffer.get(), vertices);
    Util::drawLineSegment(camPos_, bottomRight_, vec4(1, 0.5, 0, 1), indexBuffer.get(), vertices);
    Util::drawLineSegment(camPos_, topRight_, vec4(1, 0.5, 0, 1), indexBuffer.get(), vertices);

    Util::drawLineSegment(topLeft_, bottomLeft_, vec4(1, 0.5, 0, 1), indexBuffer.get(), vertices);
    Util::drawLineSegment(bottomLeft_, bottomRight_, vec4(1, 0.5, 0, 1), indexBuffer.get(),
                          vertices);
    Util::drawLineSegment(bottomRight_, topRight_, vec4(1, 0.5, 0, 1), indexBuffer.get(), vertices);
    Util::drawLineSegment(topRight_, topLeft_, vec4(1, 0.5, 0, 1), indexBuffer.get(), vertices);
}
...
void Scene::drawAxesGeometry(std::shared_ptr<BasicMesh> mesh,
    ...
    Util::drawLineSegment(vec3(0, 0, 0), vec3(1, 0, 0), vec4(1, 0, 0, 1), indexBuffer.get(),
                          vertices);
    Util::drawLineSegment(vec3(0, 0, 0), vec3(0, 1, 0), vec4(0, 1, 0, 1), indexBuffer.get(),
                          vertices);
    Util::drawLineSegment(vec3(0, 0, 0), vec3(0, 0, 1), vec4(0, 0, 1, 1), indexBuffer.get(),
                          vertices);
}
...
void Scene::render(LayerRAM* lr) const {
    ...
    LogInfo("Scene raytraced. " << count_inter << " intersections have been found, for which "
                                << count_light_on << " shading computations have been made.");
}

Ray Scene::getRay(size2_t point) const {
    return Ray(camPos_, bottomLeft_ + Util::scalarMult(point.x, right_) +
                            Util::scalarMult(point.y, up_) - camPos_);
}
